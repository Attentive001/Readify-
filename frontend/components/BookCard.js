...
export default function BookCard({ book }) {
  ...
  const cover = coverClasses[book.accent] || coverClasses.gold;
  ...
  return (
    <Link href={`/books/${book.id}`} className="block">
      <article className="group overflow-hidden rounded-2xl border border-stone-200 bg-white shadow-sm transition hover:-translate-y-1 hover:shadow-lg">
        <div
          className={`flex h-64 items-end bg-gradient-to-br ${cover} p-5`}
        >
        ...
        <div className="p-5">
          ...
          <div className="mt-4 flex items-center justify-between">
            <span className="text-xs text-stone-500">
              {book.language_name || book.language_code || "Unknown language"}
            </span>
