...
export default async function HomePage() {
  /*
   * IMPORTANT:
   * allBooks comes directly from PostgreSQL through:
   * GET /api/v1/books
   *
   * This means uploaded books such as:
   * THE BOOK OF ENOCH
   * will appear here even if they are not featured/popular.
   */
  ...
  /*
   * These sections still use the database endpoints.
   */
  ...
  return (
    <main>
        ...
        {allBooks.length === 0 ? (
          <div className="rounded-2xl border border-ink/10 bg-white/50 p-10 text-center">
            ...
            <p className="mt-2 text-sm text-ink/55">
              Upload a book from the admin page to add it to
              the Readify library.
            </p>
          </div>
        ) : (
        ...
        <div className="rounded-[2rem] bg-ink text-parchment p-8 sm:p-12 flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
          <div>
            ...
            <p className="mt-3 text-sm text-parchment/65 max-w-lg">
              Save books to your library and return to your reading
      ...
      {/* ================= RECENT ================= */}
      {recent.length > 0 && (
        <section className="max-w-7xl mx-auto px-5 lg:px-8 mt-20 pb-20">
          <div className="flex items-end justify-between mb-5">
            <div>
              <p className="text-xs uppercase tracking-[.2em] text-gold font-bold">
                Fresh arrivals
